    ...
    class PurchaseOrderGeneralDB
    {
        ...
        public List<pogeneralheader> getPOHeadersList()
        {
            ...
            try
            {
                ...
                string query = "select DocumentID,DocumentName,TemporaryNo,TemporaryDate,PONo,PODate,Reference, " +
                               "ProjectID,OfficeID,CustomerID,CustomerName from ViewPOGeneral where status = 1 and" +
                               " DocumentStatus = 99 order by PONo desc";
                SqlConnection conn = new SqlConnection(Login.connString);
                SqlCommand cmd = new SqlCommand(query, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    ...
                    pog.DocumentID = reader.GetString(0);
                    pog.DocumentName = reader.GetString(1);
                    pog.TemporaryNo = reader.GetInt32(2);
                    pog.TemporaryDate = reader.GetDateTime(3);
                    pog.PONo = reader.GetInt32(4);
                    if (!reader.IsDBNull(5))
                    {
                        pog.PODate = reader.GetDateTime(5);
                    }
                    if (!reader.IsDBNull(6))
                    {
                        pog.Reference = reader.GetString(6);
                    }
                    pog.ProjectID = reader.GetString(7);
                    pog.OfficeID = reader.GetString(8);
                    pog.CustomerID = reader.GetString(9);
                    pog.CustomerName = reader.GetString(10);
                    POGList.Add(pog);
                }
                conn.Close();
            }
