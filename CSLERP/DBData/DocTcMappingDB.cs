...
namespace CSLERP.DBData
{
    ...
    class DocTcMappingDB
    {
        ...
        public List<doctcmapping> getdocTCList(string docid)
        {
            ...
            try
            {
                ...
                string query = "select c.RowID,c.ParagraphID,c.ParagraphHeading,c.Detail from TermsAndCondition c,TCMapping d where "+
                              "  c.RowID = d.ReferenceTC and d.DocumentID = '"+docid+"' ";
                SqlCommand cmd = new SqlCommand(query, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
        ...
        public List<doctcmapping> getTCList()
        {
            doctcmapping TcSel;
            List<doctcmapping> TCList = new List<doctcmapping>();
            try
            {
                SqlConnection conn = new SqlConnection(Login.connString);
                string query = "select RowID,ParagraphID,ParagraphHeading,detail from TermsAndCondition ";
                SqlCommand cmd = new SqlCommand(query, conn);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
        ...
        public DataGridView getTclistGrid()
        {
            ...
            try
            {
                ...
                List<doctcmapping> empSElList = DoctcDB.getTCList();
